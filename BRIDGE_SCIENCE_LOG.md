...
We ran AP under 3 prediction-set definitions to test whether the pipeline's deliberate
over-coverage (conf=0.001, TOP_K=20 → ~19 masks/img) deflates AP:
...
WHAT IT DOES NOT SHOW: single-class (flower) AP, not multi-category. GT = manual TP polygons; their per-floret granularity caps high-IoU AP. mIoU "undetected=0" convention is conservative.
